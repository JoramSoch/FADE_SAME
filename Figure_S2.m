...
l2 = {'FADE/middle-aged', 'FADE/older', 'SAME/middle-aged', 'SAME/older'};
...
for i = 1:numel(m2)
    ...
    bar([1:4], [m2(i).b_est(1), 0, m2(i).b_est(3), 0], 'c');
    ...
    ylim([(11/10)*min(m2(i).b_est(1:size(c2,2))-m2(i).CIs), (-1/10)*min(m2(i).b_est(1:size(c2,2))-m2(i).CIs)]);
    set(gca,'Box','On');
    set(gca,'XTick',[mean([1,2]), mean([3,4])],'XTickLabel',{'FADE', 'SAME'});
    if i == 1, legend('middle-aged subjects', 'older subjects', 'Location', 'SouthEast'); end;
    if i == 2, legend('middle-aged subjects', 'older subjects', 'Location', 'SouthWest'); end;
    ...
    for j = 1:numel(m2(i).t2)
        if m2(i).t2(j).p >= 0.001
            text((j-1)*2+1.5, (-1/20)*min(m2(i).b_est(1:size(c2,2))-m2(i).CIs), ...
